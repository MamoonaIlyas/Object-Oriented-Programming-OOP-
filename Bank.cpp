...
double balance;
public:
Account(string title, int serial, int accNumber, double initialBalance) : accountTitle(title), serialNumber(serial), accountNumber(accNumber), balance(initialBalance) {}
...
public:
PlusAccount(string title, int serial, int accNumber, double initialBalance, double rate, double minBalance)
: Account(title, serial, accNumber, initialBalance), interestRate(rate), minimumBalance(minBalance) {}
...
public:
FreedomAccount(string title, int serial, int accNumber, double initialBalance, double fee, double interest)
: Account(title, serial, accNumber, initialBalance), transactionFee(fee), fixedInterest(interest) {}
void credit(double amount) {
balance += amount - transactionFee;
cout << "Credited " << amount << " to the Freedom account (Transaction Fee: " << transactionFee << ")." << endl;
}
void calculateInterest() {
...
cin >> minBalance;
accounts[i] = new PlusAccount(title, serial, accNumber, initialBalance, rate, minBalance);
...
cin >> interest;
accounts[i] = new FreedomAccount(title, serial, accNumber, initialBalance, fee, interest);
}
cin.ignore(); // Ignore the newline character
}
